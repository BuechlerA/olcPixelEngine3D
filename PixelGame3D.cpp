...
class GameEngine3D : public olc::PixelGameEngine
{
public:
	GameEngine3D()
	{
...
private:
	...
	// Calculate light reflected to camera
	olc::Pixel GetColour(float lum)
	{
		olc::Pixel colour = olc::Pixel(255 * lum, 255 * lum, 255 * lum, 255);
		return colour;
	}
...
{
	GameEngine3D window;
	if (window.Construct(320, 240, 2, 2))
		window.Start();
	return 0;
}